...
if ($choice -eq "n") {
    ...
    Remove-Item -Force "$desktop\Aria2下载器.lnk"
    "卸载完成"
    Pause
    exit
}
...
$dhtfile = "$dirpath\config\dht.dat"
...
"dht-file-path=$dhtfile" >> $ConfigFile
...
Copy-Item -Force .\dht.dat $dhtfile
...
if ($startUp -eq "y") {
    ...
    Copy-Item .\Aria2StartUp.vbs $dirpath
