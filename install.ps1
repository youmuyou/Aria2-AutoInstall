...
(Invoke-webrequest -URI "http://tracker.tbuu.xyz/").Content > BackupTracker.txt
...
if ($startUp -eq "y") {
    ...
    (Get-Content Aria2StartUp.ps1) -replace 'Replace', "`"$dirpath`"" | Set-Content .\Aria2StartUp.ps1
    Copy-Item .\Aria2StartUp.ps1 $startUpFolder
    .\Aria2StartUp.ps1
} else {
    "如你所愿"
    Start-Sleep -Seconds 1
